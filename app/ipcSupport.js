...
ipcMain.on("cycle-repeat", (event, arg) => {
  switch (arg) {
    case "off":
      spotifyApi.setRepeat({ state: "context" }).then(
        () => {},
        (err) => {
          catch_error(err);
        }
      ...
      break;
    case "context":
      spotifyApi.setRepeat({ state: "track" }).then(
        () => {},
        (err) => {
          catch_error(err);
        }
      ...
      break;
    case "track":
      spotifyApi.setRepeat({ state: "off" }).then(
        () => {},
        (err) => {
          catch_error(err);
        }
...
ipcMain.on("search", (event, args) => {
  ...
  spotifyApi.search(args, ["track"], { limit: 1 }).then(
    function (data) {
